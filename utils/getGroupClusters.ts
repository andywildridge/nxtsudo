...
export const getGroupClusters = (possibles:  ReadonlyMap<number, Set<number>>, groupings: Collections) => {
            ...
            if(obj.positionCluster.length === obj.canContainNumbers.size) { 
                ...
                let canRemoveInnerFind = related.filter((i: { idx:number, vals: Set<number> | undefined }, idx: number) =>{
                    if(i.vals && obj.positionCluster.includes(idx)){
                        let trim = [...i.vals].filter(i=>[...obj.canContainNumbers].includes(i));
                        if(i.vals && trim.length){
                            return true;
                        }
                    }
                }) as Array<{ idx:number, vals: Set<number>}>;
                let canRemoveInner = canRemoveInnerFind.map((i: { idx:number, vals: Set<number> })=>({
                    idx: i.idx,
                    vals: [...i.vals].filter(i=>![...obj.canContainNumbers].includes(i))
                }));
                let canRemoveOuter = related.filter((i: { idx:number, vals: Set<number> | undefined }, idx: number)=>{
                    if(i.vals && !obj.positionCluster.includes(idx)){
                        let trim = [...i.vals].filter(i=>[...obj.canContainNumbers].includes(i));
                        if(trim.length){
                            return true;
