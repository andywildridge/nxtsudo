...
export default function Home() {
    ...
    const solveSquare = (idx: number) => {
        const { initialClues, solved, possibles, solvable, removable } =
            sudoState;
        if (!solvable[idx]) {
            return false;
        }
        ...
        groupIndeces["row"](indexToRow(idx).idx).forEach((i: number) =>
            removePoss(i, [num], possibles)
        );
        groupIndeces["col"](indexToCol(idx).idx).forEach((i: number) =>
            removePoss(i, [num], possibles)
        );
        groupIndeces["box"](indexToBox(idx).idx).forEach((i: number) =>
            removePoss(i, [num], possibles)
        );
                ...
                <div className="w-[400px] m-auto ">
                    <div className="[&>*:nth-child(odd)]:border-dashed [&>*:nth-child(odd)]:border-2 [&>*:nth-child(odd)]:border-indigo-600 bcontainer m-auto grid grid-cols-9">
                        {grid.map((i: undefined, idx: number) => {
                            ...
                            const possibles = [
                                ...(sudoState.possibles.get(idx) || []),
                            ]; // handle destrucure undefined
                            const squareStyle = `${
                                sudoState.initialClues?.includes(idx)
                                    ? "font-bold"
                                    : ""
